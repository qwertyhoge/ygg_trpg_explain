...
function CharacterMaking(props){
    ...
    return <PageBase header={props.header}>
        ...
        <ProgressCard title="タイプ決定">
            ここで決定するタイプは活性タイプ、身体タイプ、能力タイプの三つ。活性は暴走率上昇でどんなメリットを受けるか、身体タイプはどのステータスが1伸びるか、能力タイプは伸びるステータスに影響する。身体、能力タイプにシステム上の制限はないが、キャラ設定に合ったものを選んでほしい。
        </ProgressCard>
        <ProgressCard title="パラメータ決定・算出">
            10pと5p相当のキャラ特性(フレーバー付き割り振り)を自由に6種のパラメータに振り分けることができる。また、ステータスに応じて格闘・武器・能力攻撃力を調整できる。キャラシに適切に書き込めば、各種数値の算出は勝手にやってくれる。
        </ProgressCard>
        ...
        <ProgressCard title="装備等決め">
            使用制限付きのやつとかを決める。キャラごとの汎用切り札となるSPE、シナリオごとの汎用切り札となるカルマ、能力を使わずとも特殊攻撃や特殊効果を得られる特殊武装、回復アイテムなどの所持品を設定する。ノーマルは能力がない分RPの幅が少なくなるが、特殊武装のアタッチメントでキャラクターらしさを出せるはず。
        </ProgressCard>
    </PageBase>
}
